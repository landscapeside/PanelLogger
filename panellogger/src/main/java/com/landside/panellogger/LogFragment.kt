...
import com.landside.panellogger.JSONS.isJson
...
class LogFragment : Fragment(R.layout.fragment_log) {
    ...
    inner class LogPriorityAdapter(
        ...
        inner class PriorityHolder(val itemBinding: ItemLogPriorityBinding) :
    ...
    inner class LogAdapter : RecyclerView.Adapter<LogHolder>() {
        ...
        override fun onBindViewHolder(
            ...
            val textColor = ResourcesCompat.getColor(
                resources, logItem.priority.textColor, null
            )
            holder.container.tv_tag.setTextColor(textColor)
            holder.container.tv_tag.text = logItem.tag ?: "Unknown"
            holder.container.tv_divider.setTextColor(textColor)
            holder.container.tv_msg.setTextColor(textColor)
            holder.container.tv_msg.text = logItem.message
            holder.container.log_box.setOnClickListener {
                val clipboard: ClipboardManager =
                    requireContext().getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
                val clipData: ClipData
                if (logItem.message.isJson()) {
                    clipData = ClipData.newPlainText(
                        null,
                        JSONS.parseJson(
                            logItem.copy(
                                message = JSONS.parseJsonObject(logItem.message)?.toString() ?: ""
                            )
                        )
                    )
                } else {
                    clipData = ClipData.newPlainText(null, JSONS.parseJson(logItem))
                }
